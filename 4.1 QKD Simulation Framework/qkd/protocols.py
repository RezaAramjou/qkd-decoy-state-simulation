...
@dataclass(frozen=True)
class PoissonSource:
    """Configuration for a light source emitting pulses."""
    pulse_configs: Sequence[PulseConfig]

    def __post_init__(self):
        if not self.pulse_configs:
            raise ParameterValidationError("PoissonSource must have at least one pulse configuration.")
        if len({pc.name for pc in self.pulse_configs}) != len(self.pulse_configs):
            raise ParameterValidationError("PulseConfig names must be unique within a source.")
        probs = np.array([pc.probability for pc in self.pulse_configs], dtype=float)
        if not np.all(np.isfinite(probs) & (probs >= 0)):
            raise DomainError("All pulse probabilities must be finite and non-negative.")
        prob_sum = np.sum(probs)
        if not np.isclose(prob_sum, 1.0):
            logger.warning(f"Pulse probabilities sum to {prob_sum}, renormalizing to 1.0.")
        if prob_sum <= 0:
            raise DomainError("Sum of pulse probabilities must be positive.")
...
class BB84DecoyProtocol(Protocol):
    ...
    def __init__(self, alice_z_basis_prob: float, bob_z_basis_prob: float,
                 source: PoissonSource, double_click_policy: DoubleClickPolicy):
        ...
        if not isinstance(double_click_policy, DoubleClickPolicy):
            raise TypeError("double_click_policy must be a DoubleClickPolicy instance.")
    ...
    def sift_results(self, prepared_states: BB84PreparedStates,
                     detection_results: DetectionResults, rng: RNG) -> SiftingResults:
...
is a successful Psi- projection. A realistic model requires a sophisticated
    ...
    def sift_results(self, prepared_states: MDIPreparedStates,
                     detection_results: DetectionResults, rng: RNG) -> SiftingResults:
...
if __name__ == '__main__':
    ...
    source_config = PoissonSource(pulse_configs=[
        PulseConfig(probability=0.8, mean_photon_number=0.5, name='signal'),
        PulseConfig(probability=0.1, mean_photon_number=0.1, name='decoy'),
        PulseConfig(probability=0.1, mean_photon_number=0.0, name='vacuum'),
    ])
