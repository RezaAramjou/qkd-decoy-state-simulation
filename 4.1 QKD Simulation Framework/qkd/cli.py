...
def save_results_atomically(results: Any, output_path: Path, save_seed: bool, is_debug: bool) -> None:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    try:
        ...
        try:
            system = QKDSystem(
                params, seed=master_seed, save_master_seed=args.save_seed,
                termination_event=TERMINATION_REQUESTED, run_metadata=run_metadata
            )
        except TypeError:
            logger.debug("QKDSystem does not accept extended args; trying fallback constructor.")
            system = QKDSystem(params, seed=master_seed, save_master_seed=args.save_seed)
            if hasattr(system, "set_run_metadata"): system.set_run_metadata(run_metadata)
            if hasattr(system, "set_termination_event"): system.set_termination_event(TERMINATION_REQUESTED)
        ...
        if args.output:
            ...
            try:
                results_json = json.dumps(results.to_serializable_dict(), indent=2, sort_keys=True, ensure_ascii=False)
            except (TypeError, OverflowError):
