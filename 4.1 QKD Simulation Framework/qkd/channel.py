...
# Create a portable fallback exception if the project-specific one isn't found.
try:
...
# Conditionally select the dataclass decorator to ensure backward compatibility
# with Python versions older than 3.10, which do not support `kw_only`.
if sys.version_info >= (3, 10):
    _dataclass_decorator = dataclass(frozen=True, slots=True, kw_only=True)
else:
    _dataclass_decorator = dataclass(frozen=True, slots=True)
